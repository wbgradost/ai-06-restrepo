...
- `capital_cheaper_below_equilibrium_thresholdSpec` -> `capital_cheaper_below_equilibrium_threshold`: Task-allocation implication at the cost threshold following equation (6), Section 2.2, page 8, equation (6) and the two paragraphs following it.
-/

namespace AR18RaceManMachine

/--
Task-allocation implication at the cost threshold following equation (6)

Paper statement: For all tasks i below the cost threshold, R is less than W divided by gamma(i); the equilibrium threshold is the minimum of the automation and cost thresholds.

Source location: Section 2.2, page 8, equation (6) and the two paragraphs following it
...
-/
def capital_cheaper_below_equilibrium_thresholdSpec : Prop :=
  ∀ (γ : ℝ → ℝ) (w r automationThreshold costThreshold i : ℝ), StrictMono γ → (∀ j, 0 < γ j) → 0 < w → 0 < r → w / r = γ costThreshold → i < min automationThreshold costThreshold → i ≤ automationThreshold ∧ r < w / γ i
